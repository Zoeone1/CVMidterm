...
def create_model():
    model = models.resnet18(weights=None)  # 设置weights=None，使用随机初始化权重
...
def train_model(model, train_loader, test_loader, criterion, optimizer, scheduler, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > best_test_acc:
            ...
            torch.save(model.state_dict(), 'best_model_random_init.pth')
...
def plot_history(history, hyperparams, save_path='./training_plots_random'):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('损失')
    plt.title(f'随机初始化-训练和测试损失曲线 (BS={hyperparams["batch_size"]}, LR={hyperparams["lr"]})')
    plt.legend()
    plt.grid(True)
    ...
    plt.xlabel('Epoch')
    plt.ylabel('准确率')
    plt.title(f'随机初始化-训练和测试准确率曲线 (BS={hyperparams["batch_size"]}, LR={hyperparams["lr"]})')
    plt.legend()
    plt.grid(True)
    
    plt.tight_layout()
    plt.savefig(f'{save_path}/random_training_history_bs{hyperparams["batch_size"]}_lr{hyperparams["lr"]}.png')
    plt.close()
    ...
    for batch_size in hyperparam_grid['batch_size']:
        ...
        for lr in hyperparam_grid['learning_rate']:
            for epochs in hyperparam_grid['num_epochs']:
                print(f"\n=== 随机初始化模型 - 训练配置: Batch Size={batch_size}, Learning Rate={lr}, Epochs={epochs} ===")
                ...
                if history['best_test_acc'] > best_accuracy:
                    ...
                    torch.save(model.state_dict(), 'best_overall_model_random_init.pth')
